...
class SkipFrame(gym.Wrapper):
    """
    Skip frames by repeating the same action for n frames.
    This helps the agent learn by reducing the frames to process.
    """
    
    def __init__(self, env, skip=4):
        """
        Initialize the skip frame wrapper.
    ...
    def step(self, action):
        ...
        for i in range(self._skip):
            ...
            # If the episode is done, we don't need to skip any more frames
...
class FrameStack(gym.Wrapper):
    """
    Stack n frames together to provide more context to the agent.
    This gives the agent some notion of motion and history.
    """
    
    def __init__(self, env, num_stack=4, policy_type="cnn"):
        """
        ...
        Args:
            ...
            policy_type: Type of policy network to use (affects observation space)
        """
        super().__init__(env)
        ...
        # Handle different observation space depending on policy type
        env_shape = env.observation_space.shape
        
        if policy_type == "cnn":
            # For CNN policies, maintain the 2D structure but add channel dimension
            self.observation_space = spaces.Box(
                low=0,
                high=255,
                shape=(num_stack, *env_shape),
                dtype=np.uint8
            )
        else:
            # For MLP policies, flatten the observation space
            flat_dim = int(np.prod(env_shape)) * num_stack
            self.observation_space = spaces.Box(
                low=0,
                high=255,
                shape=(flat_dim,),
    ...
    def _get_stacked_observation(self):
        """
        Stack the frames based on policy type.
        ...
        if self.policy_type == "cnn":
            # For CNN, keep the 2D structure with frames as channels
            stacked_frames = frames_array
        else:
            # For MLP, flatten everything
            stacked_frames = frames_array.flatten()
    ...
    def step(self, action):
        ...
        Returns:
            Tuple of (stacked_observation, reward, done, truncated, info)
        """
